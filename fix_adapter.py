...
class DPTHead(nn.Module):
    def __init__(self, in_channels, features=256):
        ...
        # Resize logic:
        # Map 0 (High Res) -> x4 upsample
        # Map 1 (Mid Res)  -> x2 upsample
        # Map 2 (Low Res)  -> Identity
        # Map 3 (Tiny Res) -> MaxPool
    ...
    def forward(self, features):
        ...
        # --- THE FIX: Calculate Target Size from the LARGEST Map ---
        # We process the first map (index 0) to establish the high-res target (128x128)
        f0 = features[0]
        x0 = self.projects[0](f0)
        x0 = self.resize_layers[0](x0)
        target_H, target_W = x0.shape[-2:]

        # Now process all maps and force them to match the high-res target
...
class DINOv3_DA3_Hybrid(nn.Module):
    def __init__(self):
        ...
        print("Model Configuration: Backbone Frozen EXCEPT Last Block (23) and Norms.")
        self.head = DPTHead(in_channels=[1024, 1024, 1024, 1024])

    def forward(self, x):
        features = self.backbone.get_intermediate_layers(x, n=4, reshape=True)
        depth = self.head(features)
        return depth
'''

with open('da3_adapter.py', 'w') as f:
    f.write(code)

print("SUCCESS: da3_adapter.py overwritten with HIGH-RES Fusion Logic.")